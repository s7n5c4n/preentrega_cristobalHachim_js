...
function actualizarSaldo() {
    ...
    if (balanceElement) {
        if (usuarioActual && usuarioActual.balance !== undefined) {
            balanceElement.textContent = usuarioActual.balance.toFixed(2);
        } else {
            balanceElement.textContent = 'N/A';
        }
    } else {
        console.error('Elemento currentBalance no encontrado');
    }
}
...
function actualizarUltimaTransaccion() {
    ...
    if (lastTransactionElement) {
        if (usuarioActual && usuarioActual.transacciones && usuarioActual.transacciones.length > 0) {
            const ultimaTransaccion = usuarioActual.transacciones[usuarioActual.transacciones.length - 1];
            lastTransactionElement.textContent = `${ultimaTransaccion.tipo}: $${ultimaTransaccion.monto.toFixed(2)}`;
        } else {
            lastTransactionElement.textContent = 'N/A';
        }
    } else {
        console.error('Elemento lastTransaction no encontrado');
    }
}
...
function actualizarHistorialTransacciones() {
    ...
    if (transactionTable) {
        ...
        if (usuarioActual && usuarioActual.transacciones) {
        ...
        console.error('Elemento transactionTable no encontrado');
    }
}
